...
class TestCreateUser:
    ...
    def test_create_user(self):
        with allure.step("Создать пользователя с валидными данными"):
            response, token, _ = UserAPI.create_user()
    ...
    def test_create_user_already_exists(self, create_and_delete_user):
        data = create_and_delete_user[1]
        with allure.step("Создать пользователя с данными которые уже есть в базе данных"):
            second_response, _, _ = UserAPI.create_user_with_data(data)
        with allure.step("Проверить ответ сервера"):
    ...
    def test_create_user_invalid_data_not_field(self, data_user):
        with allure.step(f"Тестовые данные: {data_user}"):
            response, _ , _ = UserAPI.create_user_with_data(data_user)
        with allure.step("Проверить ответ сервера"):
            second_response_data = response.json()
            expected_response = ExpectedResponses.create_user_missing_fields
            assert response.status_code == StatusCodes.forbidden, f"Ожидался статус {StatusCodes.forbidden}, \
